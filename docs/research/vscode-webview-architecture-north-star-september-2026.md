...
`DBTProject.validateSql` called `executionInfrastructure.createPythonBridge` and was the only extension-owned direct Python bridge caller. Step 7.1 removes it; `getPythonBridgeStatus` reporting is also removed. The adapter and Fusion package still require the Python bridge payload and `PythonEnvironment` until step 7.1 retires the adapter; the bridge payload is deleted at step 8.2.
...
**Documentation editor.** Keep. 3,890 lines of this repository's own code. Its vendored couplings are two: `CodeBlock` through `@uicore` and one `noop` from `antd/es/_util/warning`; the `ColumnLineage` type import left with documentation propagation. It does **not** import `DbtDocsRenderer` — the only reference to that 2,275,618-byte file anywhere in `src/` is a lazy `import()` inside the vendored `main.js`, so it leaves with the bundle and needs no replacement.
...
| Deleting the Python bridge assets in v1                                  | Step 7.1 removes extension-owned callers (validateSql). Adapter and Fusion retain bridge use until step 8.2. Asset copy deletion aligns with adapter retirement, not earlier.                                                  |
...
6. **Keep the Python bridge assets until step 8.2.** The adapter and Fusion package require them through their own operations. Step 7.1 removes extension-owned callers (validateSql). Any earlier deletion breaks adapter/Fusion integration.
