...
- **Build and package:** bundle the extension host, externalize `vscode`, type-check separately, and isolate webview builds.[^9] Treat the current implementation as an incremental baseline, not the target architecture. Evaluate the 2026 extension and webview landscape without preserving inherited framework, module-format, state, styling, or bundler choices; select a north star by functionality and measured performance on Cursor and VS Code 1.128, then split low-cost current-refactor improvements from larger v2 replacements. Keep the VSIX platform-neutral while dbt remains external; introduce target-specific packages only if native artifacts enter the VSIX.[^10]
