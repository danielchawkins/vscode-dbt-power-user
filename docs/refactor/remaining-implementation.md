...
**Completed at this tip:** 1.2 version gate, 1.3 conflict guard and `enabled`, all of Phase 2, steps 3.1 through 3.15, and Phase 4.1 through 4.3. Project Context now selects Declared Projects for Query Manifest consumers without changing their signatures; the old discovery path remains the sole DBTProject producer until 4.4. The extension packages as `0.1.0-alpha.0` against the 1.128 Extensions API with webview compiled for Chromium 148.

**Completed evidence:** pinned VS Code 1.128.0 and unauthenticated Cursor 3.21.16 each record all three host activation phases, real-webview FCP, and host resolve-to-ready across ten fresh processes. Webview target is now `chrome148` with `rolldownOptions`, and the Tailwind guard retains 543 base selector tokens. Project Registry has 21 tests covering registration, fail-closed semantics, explicitly declared package roots, canonical deduplication and aliases, lookup order, containment, watchers, configuration and workspace-folder changes, stable events, and disposal.

**Next:** Phase 4.4 retires recursive discovery and rewires DBTProject production onto the registry.
...
| 4.4  | `feat/retire-discovery`             | Delete `src/dbt_client/dbtWorkspaceFolder.ts`. `DBTProjectDetection` and its implementations are exports of `@altimateai/dbt-integration`, so remove their imports, container bindings, and composition in `src/inversify.config.ts` — there are no local class files to delete. Remove `dbt.allowListFolders` and Query Manifest's temporary zero-registry fallback; replace obsolete workspace-folder tests with Project Registry coverage and remove `it.failing`. |
| 4.5  | `feat/retire-project-pin-fallbacks` | Replace silent `dbtPowerUser.projectSelected` resolution in hover, autocomplete, code lenses, and path-free SQL execution with Project Context inference or an explicit user-invoked pick. Preserve explicit project-pick and validate/install behavior. Release `0.2.0-alpha.0`.                                                                                                                                                                                     |
