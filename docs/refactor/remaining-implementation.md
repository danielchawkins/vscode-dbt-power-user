...
**Completed at this tip:** 1.2 version gate, 1.3 conflict guard and `enabled`, all of Phase 2, steps 3.1 through 3.15, and Phase 4.1 through 4.4. Project Registry is the sole scope authority, DBTProjectContainer produces one DBTProject per Declared Project, and Query Manifest maps only through Project Context. Recursive discovery, `dbt.allowListFolders`, and the transitional zero-registry fallback are gone. The extension packages as `0.1.0-alpha.0` against the 1.128 Extensions API with webview compiled for Chromium 148.
...
**Next:** Phase 4.5 retires silent `dbtPowerUser.projectSelected` resolution fallbacks.
