...
**Completed at this tip:** Phases 0–4, the Fusion client through 5.4, publication epochs, the manifest metadata-source port, Phase 7.1 adapter retirement, and Phase 8.2 Python bridge removal. Project Registry is the sole scope authority and DBTProjectContainer routes one metadata source per Declared Project through the existing consumer event. Core, Cloud, and Core-command construction is removed. `FusionProjectIntegration` composes the published Fusion integration directly with source-path and `dbt_project.yml` watchers only; run history reads `run_results.json` after queued commands only when post-command content differs from the pre-command snapshot. Direct Fusion CLI execution uses the same `ConfiguredFusionExecutableResolver` path and env snapshot as the LSP client through a per-project process boundary; folder-scoped executable-path changes re-resolve and replace the CLI delegate while keeping LSP and CLI synchronized; the Python bridge payload, sqlfmt document formatter, and Python extension dependency are gone. The extension packages as `0.2.0-alpha.0` against the 1.128 Extensions API with webview compiled for Chromium 148. Phase 7's operation-routing steps (7.2, 7.3, 7.4, 7.7) are verified against that CLI path rather than re-architected: compile/run/build/test/clean/deps argument construction and progress-token cancellation are unit-tested, including a real, unmocked child process proving `SIGTERM` actually terminates it and that stdout reaches the terminal; query-result column type metadata is reported as unknown instead of forwarding the published integration's fabricated `"string"`, while real row values (numbers, booleans, nulls) reach the query panel's Perspective schema inference unforced; and per-project defer settings can no longer reach the published integration's hosted/remote manifest path. `ConfiguredFusionCommandProjectIntegration` now overrides `getDeferParams` to emit `--defer --state <dir>` (plus `--favor-state` when set) against the configured local state directory for `run`/`build`/`test`/`compile`, accepting either a directory or a `manifest.json` file and resolving a relative `manifestPathForDeferral` against the project root; verified accepted by the mise-pinned Fusion 2.0.5 binary, including a deferred run succeeding once the referenced table exists. `--no-defer` remains when disabled, and an enabled-but-unusable path (missing, empty, or a non-`manifest.json` file) logs a terminal warning and emits nothing rather than throwing. Query preview (`dbt show --inline`) and CTE preview/ad hoc compiles build and execute their command entirely inside the published library method with no seam to add arguments, so they stay undeferred. `DBTProject.compileModel` now routes through the same `prepareAndQueue` error handling as the other queued commands so a compile failure reaches the terminal instead of an unhandled rejection.
