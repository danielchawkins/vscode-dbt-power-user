...
What is still true of the product: it is manifest-driven. Extension-host Altimate API/network egress and `AltimateRequest` are removed; local column lineage returns unavailable when the native SQL engine fails, becomes unavailable, or is cancelled (panel status false, no error popup on cancellation). `DBTProjectIntegrationAdapter` remains only for manifest parsing and its private watcher; Core and Cloud are not constructible. `AltimateHttpClient` and `DBTConfiguration` remain only as transitive dependencies of `ModelDepthParser` in `@altimateai/dbt-integration`; wrap them, never patch `node_modules`.
