...
**Next:** step 5.6 — delete the legacy manifest-backed `autocompletion_provider`, `definition_provider`, and `hover_provider`, each gated on its LSP-backed replacement's production-shaped flow test and the editor-flow evidence in `fusion-editor-flow-evidence.md`. Then Phase 10, consumer adoption in the finance-pipelines Consumer Repository, then the 1.0.0 release.
