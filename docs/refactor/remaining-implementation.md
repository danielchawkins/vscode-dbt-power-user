...
| S1    | 5.5               | `docs/adr/0005-dependency-diagnostics.md`. 2026-09-22 opt-in rerun: parse-clean control, null load probes (`getProjectInfo`/`completion`/`definition`), post-`listNodes` Computing Lineage only; no `publishDiagnostics`. **D3** stays provisional. Gate not discharged.                                                                                                                                                                                              |
| S10   | 5.6, 7.4          | `docs/refactor/s10-static-analysis.md`. Inconclusive: neither mode loaded within five minutes; short 2.0.5/2.0.6 controls and a confounded official-extension run did not reproduce the historical loaded capture. Configured-mode plumbing may proceed; provider deletion and strict-dependent features remain gated.                                                                                                                                                |
| S8    | 6.0               | `docs/refactor/s8-config-changes.md`. Inconclusive: the server registered `**/*`, and neutral project/profile edits plus an in-place lock rewrite were applied, but the project never loaded and no producer evidence appeared. Step 6.0's event list remains open.                                                                                                                                                                                                   |
| S7    | 7.1               | `docs/refactor/s7-target-writes.md`. Inconclusive: the project never loaded, no edit/save occurred, and snapshots were before/after only. No target write was observed, so the watcher-rate arm was not triggered. Step 7.1 remains open.                                                                                                                                                                                                                             |
...
Then serial 5.0 → 5.4. **5.0 follows D5**: `src/fusion/staticAnalysisMode.ts` carries the resource-scoped configured mode, supplies the launch argument, and keeps effective mode unknown without positive server evidence. There is no login or fallback branch. 5.1 is the first production configured-path resolver (`src/fusion/fusionExecutable.ts`). 5.2 is reverse-socket (`src/lsp/reverseSocketTransport.ts`), vscode-free except `Disposable`. 5.3 is client + pool with mandatory `--command-prefix`. 5.4 is status/output, including the unknown-effective-mode surface, and the permanent zero-notification spy test. **D3** gates 5.5's diagnostics policy. A reproducible loaded-project S10 control gates 5.6; 5.6's own integration tests must exercise every editor flow before deleting the five provider directories. The `0.3.0-alpha.0` release follows both gates.
...
| Before 5.5     | S1 / **D3** — 2026-09-22 capture: parse-clean control but no positive load probe; no diagnostics observed                                                                                                                 |
