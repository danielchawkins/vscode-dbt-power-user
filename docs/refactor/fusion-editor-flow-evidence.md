...
- **Symlink hazard — reproduces through the extension; fixed in the client** (see `official-client-contracts.md`, Symlinked Project Roots): `probeSymlinkedRoot` in `fusionEditorFlowsCapture.test.ts` (2026-09-23) shows that `dbt lsp` driven with a symlinked `--project-dir` and symlinked document URIs stays at `models_count:0, is_estimate:true`, while the real path reaches `models_count:7`. Once the legacy definition provider was deleted, `symlinkedWorkspace.test.ts` failed the same way through the extension. `canonicalProjectRoot` in `fusionLanguageClient.ts` now launches Fusion on the realpath and remaps document URIs.
