...
11. Retain in the comprehensive target: standard LSP features; compile / preview / run / build / test; model lineage; model trees; local documentation editor; query results, CTE preview and profiler, charting; local model generation and defer where Fusion supports them.
...
- `src/dbt_client/event/manifestCacheChangedEvent.ts` declares `ManifestCacheProjectAddedEvent`, carrying the project handle, the eleven metadata maps whose types (`NodeMetaMap`, `MacroMetaMap`, `SourceMetaMap`, `GraphMetaMap`, `TestMetaMap`, and the rest) come from `src/dbt_integration/domain.ts`, and `modelDepthMap`. Every panel, tree view, lineage view, and code lens consumes it through `src/services/queryManifestService.ts`, which keys by project root and serves `getEventByCurrentProject()` and `getEventByDocument()`. The interface is declared in this repository, so stamping it is a local change.
- **Manifest parsing and source watching live in `FusionProjectIntegration`.** `src/inversify.config.ts` constructs the published Fusion integration through a local composition in `src/dbt_client/fusionProjectIntegration.ts`, consumed by `src/dbt_client/dbtProject.ts`. It owns the parsers, emits `MANIFEST_PARSED`, and watches model, macro, seed, and `dbt_project.yml` paths only. Parsers take a `ManifestProject`, which `FusionProjectIntegration` implements.
...
**Keep the existing metadata event as the internal boundary.** `ManifestCacheProjectAddedEvent` plus its map types is already the boundary, and `QueryManifestService` is already the mediator. `ProjectMetadataSource` now wraps the manifest producer without changing consumers. Fusion's LSP payload cannot populate the complete event, so no LSP metadata source or producer flip is retained. Adapter retirement must preserve the parser projection behind this port without introducing a competing snapshot interface.

**Adapter retirement removed the duplicated state path.** Step 7.1 replaced the external adapter composition with local `FusionProjectIntegration`, keeping the parser projection behind `ProjectMetadataSource` without a competing snapshot interface. The ambient target watcher is gone; Python payload and environment settings remain until step 8.2.
...
S10 did not establish the runtime difference between `baseline` and `strict`. Step 5.0 records the configured mode and supplies the launch argument; effective mode stays `unknown` without positive server evidence, and configuration alone never enables a strict-only capability. **Superseded:** the 2026-09-26 evidence (section 6) establishes the difference, and step 2 of [column-lineage-ship-plan.md](column-lineage-ship-plan.md) deletes the effective mode and the capability helpers. Provider replacement proceeds behind production-shaped tests of the capabilities Fusion advertises. Warehouse- and strict-dependent enhancements remain deferred until positive runtime evidence.
...
Contract: `just test-integration` requires the pinned Fusion on `PATH` and skips with a clear message otherwise; it never runs inside `just check`. Fusion looks for `profiles.yml` in `--profiles-dir`, then `DBT_ENGINE_PROFILES_DIR` (which supersedes `DBT_PROFILES_DIR`), then the working directory, then `~/.dbt`; `--project-dir` does not move that lookup (`docs/research/dbt-global-configs-flags-september-2026.md`). The harness therefore starts the extension host with `--force-disable-user-env`, so VS Code does not re-export the developer's login-shell profiles variables, and points both variables at the fixture copy. The extension itself adds no profiles flag unless `fusionPowerUser.profilesDir` is set.
...
**5.6 — Delete the language providers.** Delete each inherited provider with the production-shaped integration test for its replacement in the same implementation PR. Delete `src/autocompletion_provider/`, `src/definition_provider/`, `src/hover_provider/`, and `src/document_formatting_edit_provider/` after their completion, definition, hover, formatting, and code-action flows pass. The legacy Python-backed sqlfmt document formatter and its integration tests were removed in step 8.2 once LSP formatting replaced that path. `src/validation_provider/` (hosted credential validation) is removed; `dbtPowerUser.validateSql` was an unsupported Python-bridge command removed at step 7.1 (outside product boundary); standard LanguageClient diagnostics arrive separately, tested by their own provider flows. Remove `dbt.sqlFmtPath` and `dbt.sqlFmtAdditionalParams`. In `src/code_lens_provider/`, keep `cteCodeLensProvider.ts`, `virtualSqlCodeLensProvider.ts`, and `documentationCodeLensProvider.ts`; the fate of `sourceModelCreationCodeLensProvider.ts` follows model generation in Phase 7.
...
**6.2 — Metadata capability result — complete without an LSP source.** `dbt.getProjectInfo`, `dbt.listNodes`, and `dbt.getCurrentNode` do not supply the complete event contract: macros, docs, exposures, tests, metrics, semantic models, depth inputs, and full parser metadata are absent. A source that always refuses to publish is dead code, so none is retained. The manifest source remains selected. Revisit only when Fusion exposes a contract-complete command payload.

**6.3 — Producer flip — deferred.** There is no complete LSP producer to select. Adapter retirement must preserve the existing parser projection behind an explicit Fusion CLI operation while removing the ambient watcher; it must not duplicate parser logic, emit partial maps, or fabricate empty maps.
...
**7.1 — Retire the external project-integration adapter — complete at this tip.** This removed the remaining parser/watcher coupling to the external adapter class. **S7** ran first; its inconclusive load evidence did not block retirement of the prohibited ambient target watcher.

Replace the external adapter composition with local `FusionProjectIntegration` in `src/dbt_client/fusionProjectIntegration.ts` that constructs the published Fusion integration directly and exposes only the operations this product retains — dropping `validateSql`, `validateSQLDryRun`, `performDatapilotHealthcheck`, `fetchSqlglotSchema`, `validateWhetherSqlHasColumns`, `getPythonBridgeStatus`, and docs generation, which Fusion does not support. Rewire `src/dbt_client/dbtProject.ts` and the factory in `src/inversify.config.ts` onto it. Metadata parser projection moved into `FusionProjectIntegration` behind the manifest metadata source; the 6.3 producer flip remains deferred. Queued commands parse `run_results.json` only when post-command content differs from the pre-command snapshot; startup deps, clean, and compile cannot republish unchanged files.

What this removes: the adapter's private ambient `fs.watch` over the target directory, which is the one prohibited artifact path in Section 2.8 and is not removable any other way. Core, Cloud, and Core-command construction was already removed with fail-closed positional factories. Justify the watcher removal as eliminating an ambient stale-and-refresh path, not as proof that artifact reading is unsafe. Python payload and environment settings remain until step 8.2.

Verify: `just check` green; production `grep -rn "DBTProjectIntegrationAdapter" src` is limited to the parser-boundary cast in `fusionProjectIntegration.ts`; no `fs.watch` or `FileSystemWatcher` over any project's target directory; compile, run, build, and test integration tests still pass.
...
**Status at this tip: routed through the Fusion CLI, not `dbt.compileFile`/`dbt.compileLsp`.** **S2** found every tried `compileFile` shape returned an error, so compile stays on `dbt compile` through the same resolved executable and env as the LSP client (`configuredFusionCommandIntegration.test.ts` asserts the spawned `command` and `envVars` match the resolved `FusionExecutable`). Revisiting the LSP route is future work behind a working `compileFile`/`compileLsp` response, not this step. Progress-token cancellation is tested at two levels: `dbtProject.test.ts` asserts the progress token's cancellation reaches the queued command's `AbortSignal`, and `configuredFusionCommandIntegration.test.ts` spawns a real, unmocked child process (`node -e`) and confirms aborting that signal actually terminates it via `SIGTERM` — the published `CommandProcessExecution.spawn()` behavior, not something this extension implements. That proves local-process cancellation; it does not prove the warehouse itself stops running the statement — see **S9**.

**7.3 — Run, build, test, clean, deps.** Keep the `DBTCommand` and queue machinery, which is already CLI-shaped, and `src/commands/runModel.ts`. Retain `runCurrentModel`, `buildCurrentModel` and its three graph-operator variants, `buildCurrentProject`, `testCurrentModel`, `runTest`, `runChildrenModels`, `runParentModels`, `cleanCurrentProject`, and `printEnvVars`. The `generateDBTDocs` command and call path are removed because the published integration's `generateDocs` throws unconditionally. Fusion 2.0.5 does run `dbt docs generate`, writing `index.html`, assets, and a parquet catalog but no `catalog.json`, so restoring the command through the CLI path remains open.

Verify: integration tests asserting argument construction for each command and that output reaches the terminal.

**Status at this tip: complete against the CLI shape.** Argument construction for `run`/`build` (including the three graph-operator variants), `buildProject`, `runTest`/`runModelTest`, `clean`, and `installDeps`, and progress-token cancellation reaching the queued command's `AbortSignal`, are covered by unit tests in `dbtProject.test.ts` against the real `DBTCommandFactory`. "Output reaches the terminal" is tested at the process boundary: `configuredFusionCommandIntegration.test.ts` runs a real command through the real, unmocked `CommandProcessExecutionFactory` and asserts its stdout reaches the terminal's `log()`. `generateDBTDocs` stays removed.
...
**Status at this tip: column type metadata is honestly unknown; row values are real.** **S2** found `show.columns` null on dummy credentials, so `dbt.show` does not resolve the type question; confirmed directly against the published integration, `DBTFusionCommandProjectIntegration.executeSQL` runs `dbt show --inline --output json` and fabricates the literal string `"string"` in `column_types` for every column regardless of its real type. Running `dbt show --inline` against a local DuckDB profile (no warehouse required) shows the row values themselves are real JSON, not stringified: `{"int_col":1,"str_col":"x","bool_col":true,"float_col":1.5,"null_col":null}`. Only the `column_types` array is fabricated. The single seam both `executeSQLWithLimit` and `immediatelyExecuteSQLWithLimit` read through, `markColumnTypesUnknown` in `fusionProjectIntegration.ts`, reports every column type as unknown (`null`) instead of forwarding that placeholder (`fusionProjectIntegration.test.ts`). The query panel no longer forces every column into a string schema on account of that: `buildPerspectiveTableInit` in `webview_panels/.../columnTypeMapping.ts` hands Perspective the row data directly when every type is unknown, so Perspective infers real types from the actual values, and falls back to an explicit schema (with BigInteger's precision safeguard) only when a type is actually known. `dbt.previewCte` routing remains unconfirmed per **S2** and is unchanged by this step. **S9**'s warehouse-side gaps — query id, in-flight cancellation, `QUERY_TAG` — remain open.

**7.5 — Model and column lineage.** **Superseded for column lineage** by [column-lineage-ship-plan.md](column-lineage-ship-plan.md) steps 3–6: the language server never writes lineage, so it comes from the Fusion CLI (evidence README sections 3 and 5). The text below is historical. Rebuild `src/services/dbtLineageService.ts`, `src/webview_provider/newLineagePanel.ts`, and `src/webview_provider/modelGraphViewPanel.ts` on `dbt.listNodes` (graph, materialization, access, resource type) and `dbt.getCurrentNode` (columns, `is_primary_key`, `parents`, `transformation_type`). Column lineage becomes a Local Capability served by Fusion. The earlier implementation, built on `@altimateai/altimate-core`, and the documentation propagation that depended on it are removed (see [`fusion-lsp-feature-disposition.md`](fusion-lsp-feature-disposition.md)); column lineage is rebuilt here without that dependency. Surface `compilation_failed`, `lineage_query_failed`, and `models_count_is_estimate` in the panel rather than failing silently. Keep `lineage_panel/` and `webview_panels/src/modules/lineage/`; retain `dbt.lineage.defaultExpansion`; remove `dbt.enableNewLineagePanel` by keeping only the new panel.
...
**Status at this tip: complete for compile/run/build/test; query preview stays undeferred.** There is no separate `deferToProdService.ts`; defer lives in `DBTProject.retrieveDeferConfigFromSettings` and `applyDeferConfig` in `dbtProject.ts`. That method resolves a settings entry's `manifestPathForDeferral` after variable substitution against the project root with `path.resolve`, so a relative path is anchored the way a user expects rather than against the process's working directory; it no longer forwards `manifestPathType` or `dbtCoreIntegrationId` — neither was ever declared in `package.json`'s settings schema — and always resolves through `ManifestPathType.LOCAL` (tested: a settings entry carrying `manifestPathType: "remote"` is not honored). `--defer`, `--state`, and `--favor-state` construction (`getDeferParams`/`addDeferParams`) exists only on the dbt Core integration class in the published library; `DBTFusionCommandProjectIntegration` inherits `DBTBaseProjectIntegration`'s trivial version, which returns `["--no-defer"]` when disabled and `[]` when enabled. `ConfiguredFusionCommandProjectIntegration` (`configuredFusionCommandIntegration.ts`) overrides that protected seam itself: disabled yields `["--no-defer"]`; enabled with a `ManifestPathType.LOCAL` `manifestPathForDeferral` that resolves to an existing directory, or to a `manifest.json` file whose containing directory is used, yields `["--defer", "--state", <dir>]` plus `--favor-state` when `favorState` is set; enabled with a path that is missing or neither of those shapes logs a `terminal.warn` naming `fusionPowerUser.defer.perProject` and the offending path (no toast) and yields nothing — a misconfigured path is a settings problem, not a command failure, so it never throws. The published `runModel`/`buildModel`/`buildProject`/`runTest`/`runModelTest`/`compileModel` already route through `addDeferParams`, which calls this overridden `getDeferParams`, so no other call site changed; `clean`, `deps`, and `debug` never call it, and neither does `executeSQL` (`dbt show --inline`, the query-preview path) or `unsafeCompileNode`/`unsafeCompileQuery` (CTE preview and ad hoc compiles) — those three build and execute their `DBTCommand` entirely inside the published method body with no seam to inject arguments into, so reaching them would mean reimplementing that body rather than composing with it. Query preview and CTE preview are therefore not deferred; only compile/run/build/test are. Remote (`ManifestPathType.REMOTE`) is not honored — local-only by design, matching the contract. `DBTProject.compileModel` now routes through the same `prepareAndQueue` error handling as `runModel`/`buildModel`/`runTest` so a compile failure reaches the terminal and run history instead of surfacing as an unhandled rejection from `DBTProjectContainer.compileModel`'s un-awaited call.

Verified against the mise-pinned Fusion 2.0.5 binary directly on a throwaway local DuckDB project: Fusion 2.0.5 accepted `--defer`, `--state`, and `--favor-state` on both `run` and `compile` — no unrecognized-argument error in either invocation. The deferred `compile --select model_b --defer --state <dir> --favor-state` succeeded and resolved `ref('model_a')` to the state manifest's relation (`"defer_verify"."main"."model_a"`). The deferred `run --select model_b` on the same flags failed with a DuckDB "table does not exist" error when `model_a` had never been materialized — a database error unrelated to argument parsing — and succeeded once `model_a` was built first; that second case is captured as an opt-in integration test, `src/test/integration/deferArgumentIntegration.test.ts`, which builds its own temp DuckDB project (skipping with an explicit reason when Fusion is absent from `PATH`) rather than depending on the fixture-conversion work in flight elsewhere. `--favor-state` is accepted on the command line but does not appear in `dbt run --help`; its effect for DuckDB is otherwise unverified. Unit coverage in `configuredFusionCommandIntegration.test.ts` asserts `--defer --state <dir>` and conditional `--favor-state` for `run`, the enabled-without-a-path no-op, the enabled-with-a-manifest.json-file case, the bad-path warning, and that `compileModel` sees the same arguments while `installDeps`/`clean` never do.
...
**8.1 — dbt Core and dbt Cloud — complete.** The Core and Cloud integration classes live in `@altimateai/dbt-integration`, so this deleted extension-owned construction rather than package files. Core, Cloud, and Core-command bindings, factories, imports, detection wiring, and unsupported-mode tests are removed. Fusion is the only real factory. `dbt.dbtCustomRunnerImport` and `dbt.installDepsOnProjectInitialization` remain until step 8.2 removes the Python initialization path.

Verify: `just check` green; no Core or Cloud symbol is imported anywhere in `src`.

**8.2 — The Python bridge and payload — complete.** Delete `dbt_core_integration.py`, `dbt_cloud_integration.py`, `altimate_packages/`, `src/dbt_client/pythonEnvironment.ts`, `src/dbt_client/runtimePythonEnvironmentProvider.ts`, the bridge creation and teardown on the execution infrastructure, and the `python-bridge` dependency. Delete the Python asset copy plugin in `rsbuild.config.ts` with them — the payload is runtime-required until 7.1 removes its callers, so it must not be deleted earlier. Remove the `ms-python.python` extension dependency and the Python launch configuration from `.vscode/launch.json`. Direct Fusion CLI execution uses the same `ConfiguredFusionExecutableResolver` path and env snapshot as the LSP client through a per-project `@altimateai/dbt-integration` boundary.

Verify: the unpacked VSIX contains no `.py` file; the extension activates with no Python interpreter selected; `just package` fails when any archive entry ends in `.py`.

**8.3 — Altimate and hosted remnants — complete.** `src/altimate.ts`, `src/test/suite/altimate.test.ts`, and the `dbt.altimateAiKey` / `dbt.altimateInstanceName` / `dbt.altimateUrl` settings are gone, along with `node-fetch` and `node-abort-controller`. `src/services/sharedStateService.ts` stayed: it is a local event emitter with no hosted state or credential, so it remains as the internal event bus.
...
**8.4 — Prune what the hosted removals orphaned in the webview — complete.** Orphaned message listeners and host handlers, plus dead marketing, AI, and unused icon exports and assets, are removed. Perspective, the lineage stack, and Tailwind generation are kept, each still gated on its own spike or D8.
...
**8.5 — Walkthroughs, quick picks, and dead contributions — complete.** By this tip `walkthroughCommands.ts` already carried no setup-wizard, dbt-install, or integration-switch path — earlier steps had reduced it to `validateProjects` and `installDeps`, both real Local Capabilities backed by `dbt debug` / `dbt deps` — so the step renamed it to `ProjectSetupCommands` (`src/commands/projectSetupCommands.ts`) rather than rewriting it, with the inversify binding and call sites updated to match. No `walkthroughs` contribution was added: the extension has no multi-step onboarding flow to narrate (ADR 0001 scopes it to a Fusion-only, account-free product), so a walkthrough page would describe a feature that does not exist. `src/quickpick/` was already pruned to the two files still referenced. The contribution audit found one real defect: `contributes["fusionPowerUser.buildModel"]` was a sibling of `contributes.menus` instead of nested inside it, leaving the "Build dbt Model" editor submenu empty; moving it under `menus` fixed the submenu. The audit also found two commands reachable from the command palette with no argument that would crash on a bare invocation — `fusionPowerUser.rerunFromHistory` and `fusionPowerUser.copyModelName`, both tree-item-only — and hid them with `menus.commandPalette` entries of `"when": "false"`. `runChildrenModels`, `runParentModels`, and `runTest` fall back to the active editor when invoked without a tree item, so they needed no change.

Verify: `commandConsistencyGuard.test.ts` asserts every `contributes.commands` entry has a registration in `src` and every registration has a contribution — no orphans in either direction — and now also asserts every `menus[*][].command` and `keybindings[].command` is contributed, every `menus[*][].submenu` is declared in `contributes.submenus` and has a matching `menus` key, `contributes` uses only a known allowlist of contribution points, and the tree-item-only commands above are hidden from the command palette. Keep it permanently; it is the cheapest guard against contribution drift.
...
**9.1 — Settings and command namespace — complete.** Every setting and command is `fusionPowerUser.*`, with no `dbt.*` fallback read and no deprecation alias anywhere in `src` or `package.json`. `docs/settings-migration.md` carries the full mapping. The table below records what changed:
...
| `dbt.lineage.showSelectEdges` / `showNonSelectEdges`                                                                                                                                                                         | removed — column-level lineage removed                                                      |
...
**9.2 — Documentation — complete.** `CONTEXT.md` carries **Fusion Client** and **Project Metadata Source**, both with real usage in `src/`. `docs/architecture.md` covers activation, Declared Projects, executable resolution, the target Fusion client pool and operation routing, the metadata port, and the panels, linking to the ADRs rather than restating them, and states plainly that the legacy manifest-backed providers still run until step 5.6. `docs/lsp-metadata-gaps.md` states the step 6.2 gap table against the `docs/refactor/lsp-commands.md` capture. `AGENTS.md` (`CLAUDE.md` is a symlink to it in this checkout) lists `just smoke` and `just release` and describes the current Fusion-only, adapter-retired construction. `README.md` leads with install-from-release instructions, the macOS-only scope, the Fusion 2.0.5 minimum, and the `PATH`-based `mise-vscode` note from step 5.1.

**9.3 — VSIX smoke test — complete.** Extended step 3.14's bash harness (`scripts/smoke/*.sh`, `scripts/smoke/run-smoke.mjs`) instead of adding a POSIX `scripts/smoke.sh`, since `run-host-smoke.sh` already printed `usage:` on `--help`; added `--force` there and to the fetch scripts to re-acquire a pinned host, and it requires `just package` first rather than packaging itself, matching the existing recipe's contract. For each host, `run-host-smoke.sh` now also runs the `multi-root` fixture with a disposable user-data directory (skipped only inside the tight single-fixture `FPU_RUNTIME_BENCHMARK` timing loop) and asserts, per Declared Project: extension activation; zero startup notifications, checked before and after; the Fusion LSP initialize handshake completing, observed as `FusionClient.state` reaching `running` through a smoke-gated test command, which only happens after `vscode-languageclient`'s full initialize/initialized exchange and is therefore evidence of a protocol round trip rather than process spawn alone; and no surviving `dbt lsp` process naming the run's disposable workspace path, polled for 5 seconds after host exit. A `FPU_SMOKE_REQUIRE_FUSION=1` flag, set by the CI smoke jobs, turns a missing Fusion binary into a failure instead of the explicit `skipped: no Fusion` result local runs keep. CI wires the new assertions into the existing `smoke-vscode`/`smoke-cursor` jobs alongside the runtime-benchmark step rather than a second job, so the per-editor result stays explicit and a Cursor-only failure is visible (**S6**).

Verify: `just smoke` green against both editors on macOS.

**9.4 — Release pipeline — complete.** `.github/workflows/release.yml` reuses the check-and-package job on a tag push and creates a GitHub Release carrying the VSIX and its SHA-256 checksum, marking a hyphenated (prerelease) tag as a prerelease. `just release` runs the local dry run. `docs/releasing.md` documents cutting a release and rollback by re-pinning.
...
2. **Retiring `DBTProjectIntegrationAdapter` is larger than the container wiring suggests.** It is both the manifest producer and command facade, and its parser projection cannot be replaced by the current LSP payload. The metadata port isolates consumers, but the manifest source remains selected. Split 7.1 by operation group if needed while preserving complete metadata; do not add a dead LSP source or duplicate parser stack.
...
- **Before Phase 8.2:** confirm beta.2 has been used against the real consumer repository long enough to trust deletion of the remaining Python, hosted, and authentication paths. Step 8.1 is exempt because Core and Cloud were already unreachable and their removal deletes no fallback.
...
Phase 6 step 6.3 (flipping the metadata producer), Phase 7 step 7.1 (retiring the adapter), Phase 8 steps 8.2 onward, Phase 9 step 9.1 (the namespace rename), and v2 step v2.3 (the lineage renderer) are expensive to undo. Step 8.1 is an early safe deletion because Core and Cloud were unreachable. Each remaining v1 hard boundary is preceded by a release; v2.3 is gated on its benchmark. Do not batch any of them with another step.
