...
Branch `fusion-lsp-client` is based on upstream `0.64.6`. No LSP code exists; `vscode-languageclient` is not a dependency. The line and contribution counts taken at fork time — ~44k lines in `src`, ~19k in `webview_panels/src`, 66 commands, 33 configuration properties — predate the Phase 3 removals and are not current. The package identity is `danielchawkins.fusion-power-user` / `Fusion Power User` at `0.1.0-alpha.0`; command IDs and settings intentionally retain their upstream namespaces until Phase 9.
...
- `src/dbt_client/dbtWorkspaceFolder.ts` implements recursive discovery: `workspace.findFiles` over `**/dbt_project.yml` with a hardcoded exclude, then an `allowListFolders` filter read **without** a folder URI, then `DBTProjectDetection.discoverProjects`. This is the code ADR 0003 replaces.
...
Fusion segment timing is **not** in this step: it needs a language client, so its records begin at step 6.0.
...
**6.0 — Publication epoch and exact-match acceptance.** Additive metadata on the existing seam, landed before the producer moves so both producers are stamped the same way.

The extension owns a monotonically increasing `publicationEpoch` per Declared Project: the counter marking one published projection of that project's state. It is internal architecture, not product vocabulary — it stays defined here and in the TSDoc on the field, and out of `CONTEXT.md`. Call it the publication epoch, never a server generation, revision counter, or snapshot version, each of which claims something the client cannot know. It is a client-side publication boundary and makes no claim about the server's internal state — a counter derived from filesystem events cannot prove anything about a closed producer, so it does not advance because a file changed. It advances on producer evidence only: a producer completes and publishes a projection, or a producer restarts. Every change class reaches it the same way, because the producer classifies the change and republishes; the client never classifies. That coverage rests on the producer republishing for every class, which **S8** establishes.

Two seams carry it, and only one of them is a data change. **Publication:** add the epoch, the producer identity, and the producer's own revision token where it has one to `ManifestCacheProjectAddedEvent` in `src/dbt_client/event/manifestCacheChangedEvent.ts`. Consumers that ignore the fields behave exactly as today; this adds no new interface, extends an existing one, and does not model a server generation. **Requests:** a wrapper around every extension-initiated LSP request records the client's current epoch, and the document version when the request is addressed to a `TextDocumentIdentifier`, **before dispatch**, then compares both against the live values **after the response completes**. The server is closed and stamps nothing; the epoch on a response is the one the client captured when it asked, never a field the server supplied. Anything claiming otherwise is a design error.

Acceptance is exact, scoped to what the request actually carries: the captured epoch must **equal** the current epoch — not greater-or-equal — and for a document-scoped request the captured version must equal that URI's current version. **Project-scoped** requests carry no URI and no version, so the epoch is the whole test; requiring a version where none exists would either block the request or invite a fabricated one. A response failing its applicable test is discarded, not reconciled. Publication is a single assignment of one frozen value, so no consumer observes a half-updated state.

`RequestCancelled` (-32800) and `ContentModified` (-32801) are ordinary control flow and never reach the user; where the server advertises pull diagnostics, honor `ServerCancelled`'s `retriggerRequest`, defaulting to true. Cancellation is advisory — it bounds what the extension displays, not what the server spends. Do not build a request-deduplication layer: `vscode-languageclient` already correlates and cancels, so the duplicate work such a layer would suppress may not exist. Build it only if **S7** measures duplicate extension-initiated work, and then key it on the epoch and the document version.

This step also starts the **Fusion segment timing** records that step 3.13's baseline could not produce without a client: structured records to an output channel and, behind an explicit setting, to a local JSON-lines file under the extension's storage URI, never into a project's `target/`. Each record carries the segment from the latency model, monotonic start and end, the captured epoch, the document version where there is one, the request kind, and an outcome of `ok`, `cancelled`, `contentModified`, or `error`. URIs are workspace-relative; SQL text and result rows are never recorded. This produces no telemetry and no network egress.

Contract: no consumer file changes. Do not debounce document synchronization; the client already sends incremental `didChange`. Refresh derived panels on save, not on keystroke.

Verify: unit tests for advance-on-publish, advance-on-restart, no-advance-on-file-event, discard on epoch mismatch, discard on document-version mismatch for a document-scoped request, and acceptance of a project-scoped request that carries no version; one segment record asserted end to end with its outcome.
