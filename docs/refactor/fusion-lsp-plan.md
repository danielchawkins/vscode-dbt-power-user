...
**Status at this tip: local-only, but the CLI never receives defer arguments for Fusion.** There is no separate `deferToProdService.ts`; defer lives in `DBTProject.retrieveDeferConfigFromSettings` and `applyDeferConfig` in `dbtProject.ts`. That method no longer forwards a settings entry's `manifestPathType` or `dbtCoreIntegrationId` — neither was ever declared in `package.json`'s settings schema — and always resolves through `ManifestPathType.LOCAL` when a project sets `manifestPathForDeferral`, so there is no route to the published integration's hosted/remote artifact path (tested: a settings entry carrying `manifestPathType: "remote"` is not honored). A terminal warning now names `fusionPowerUser.defer.perProject` when `deferToProduction` is enabled without a `manifestPathForDeferral`.

A deeper problem surfaced while adding an argument-level test: `--defer`, `--state`, and `--favor-state` construction (`getDeferParams`/`addDeferParams`) is implemented only on the dbt Core integration class in the published library. `DBTFusionCommandProjectIntegration` has no override and inherits `DBTBaseProjectIntegration`'s trivial version, which returns `["--no-defer"]` when disabled and `[]` — nothing — when enabled. Confirmed with a real, unmocked `ConfiguredFusionCommandProjectIntegration` in `configuredFusionCommandIntegration.test.ts`: a `DeferConfig` with `deferToProduction: true`, a real state directory, and `favorState: true` produces a `run` command with none of `--defer`, `--state`, or `--favor-state`. `manifestPathForDeferral` and `ManifestPathType` are therefore inert for Fusion today — enabling defer only suppresses `--no-defer`, it does not defer to anything. This is exactly the condition the contract above names ("If Fusion's local defer cannot support the retained shape, cut the feature and record it in the ADR"); this PR fixes the local-only settings honesty and adds the warning and test coverage that surfaced the gap, but does not cut the feature — that decision needs a Confirm gate, not a fix folded into review.
