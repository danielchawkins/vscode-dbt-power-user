...
- **Manifest parsing and source watching live in `FusionProjectIntegration`.** `src/inversify.config.ts` constructs the published Fusion integration through a local composition in `src/dbt_client/fusionProjectIntegration.ts`, consumed by `src/dbt_client/dbtProject.ts`. It owns the parsers, emits `MANIFEST_PARSED`, and watches model, macro, seed, and `dbt_project.yml` paths only. The external `DBTProjectIntegrationAdapter` class name survives only as a parser-boundary cast; Core, Cloud, and Core-command construction is gone.
- **The published Fusion integration does not extend Cloud.** `DBTFusionCommandProjectIntegration extends DBTBaseProjectIntegration`, a sibling of `DBTCloudProjectIntegration`, and `src/inversify.config.ts` constructs it directly with no Cloud type in the chain. Core and Cloud construction is deleted. Do not patch `node_modules`.
