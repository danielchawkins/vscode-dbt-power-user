...
- **The manifest producer and ambient target watcher are one external object.** `DBTProjectIntegrationAdapter` from `@altimateai/dbt-integration` is constructed in `src/inversify.config.ts` and consumed by `src/dbt_client/dbtProject.ts`. It owns the parsers, emits `MANIFEST_PARSED`, and runs a private non-recursive `fs.watch` over the target directory behind a 300 ms debounce; none of the watcher members is public, so the behavior cannot be disabled from here. Its constructor still requires four positional factories, but only Fusion is real; the Core, Cloud, and Core-command positions fail closed.
- **The published Fusion integration does not extend Cloud.** `DBTFusionCommandProjectIntegration extends DBTBaseProjectIntegration`, a sibling of `DBTCloudProjectIntegration`, and `src/inversify.config.ts` constructs it directly with no Cloud type in the chain. Core and Cloud construction is deleted even though the adapter remains. Retiring the adapter is still required to remove the manifest parse loop and ambient watcher. Do not patch `node_modules`.
- Hosted and Python remnants still referenced across `src`: `AltimateRequest`, `SharedStateService`, and `PythonEnvironment`. The integration classes for Core and Cloud live in `@altimateai/dbt-integration`, not in `src`, so removing them is deleting construction and dropping the dependency rather than deleting files. `ValidationProvider` (hosted credential validation) is removed.
...
**Adapter retirement removes the remaining duplicated state path.** The manifest parse loop, ambient target watcher, and Python bridge path live behind `DBTProjectIntegrationAdapter`. Core and Cloud construction no longer wait for that retirement: their required constructor positions fail closed. Step 7.1 still composes the published Fusion integration directly and removes the adapter-owned parse and watcher behavior.
...
**5.6 — Delete the language providers.** Delete each inherited provider with the production-shaped integration test for its replacement in the same implementation PR. Delete `src/autocompletion_provider/`, `src/definition_provider/`, `src/hover_provider/`, and `src/document_formatting_edit_provider/` after their completion, definition, hover, formatting, and code-action flows pass. `src/validation_provider/` (hosted credential validation) is removed; delete `dbtPowerUser.validateSql` and `src/commands/validateSql.ts` only with a green broken-model diagnostic test (standard LanguageClient diagnostics are separate). Remove `dbt.sqlFmtPath` and `dbt.sqlFmtAdditionalParams`. In `src/code_lens_provider/`, keep `cteCodeLensProvider.ts`, `virtualSqlCodeLensProvider.ts`, and `documentationCodeLensProvider.ts`; the fate of `sourceModelCreationCodeLensProvider.ts` follows model generation in Phase 7.
...
**7.1 — Retire the external project-integration adapter.** This removes the remaining parser/watcher and Python execution coupling. Run **S7** first: it establishes whether the server writes artifacts and what rate the ambient watcher actually experiences, which is the evidence for how the removal is described and for whether anything needs escalating rather than sequencing.
...
**8.1 — dbt Core and dbt Cloud — complete before adapter retirement.** The Core and Cloud integration classes live in `@altimateai/dbt-integration`, so this deletes extension-owned construction rather than package files. Core, Cloud, and Core-command bindings, factories, imports, detection wiring, and unsupported-mode tests are removed. The adapter's unused positional factory slots receive one fail-closed `never` factory; Fusion remains the only real factory. `dbt.dbtCustomRunnerImport` and `dbt.installDepsOnProjectInitialization` remain until the adapter and its initialization path retire.
