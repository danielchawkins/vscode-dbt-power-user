...
Ad hoc follow-up probes (not checked in) against `single-project` converted to a `duckdb` profile, spawned with the project root as `cwd`, `DBT_LSP_USE_TARGET_LSP=1`, and a valid `base` → `child` chain (`broken_ref` removed, matching the convention every checked-in integration test already uses for this fixture). See `docs/refactor/fusion-editor-flow-evidence.md` (Working-DuckDB run) for the full method and the fixed harness.
...
- `dbt.listNodes` `[]` and `dbt.getCurrentNode` `[{"uri":...,"line":0,"character":20}]` both still returned `null` on this fully loaded project (verified via `getProjectInfo`'s accurate count above), using only the documented argument shapes. **Resolved 2026-09-23**, not a Fusion server limitation: static inspection of the official `dbtlabsinc.dbt-0.104.0-universal` bundle (`docs/refactor/official-client-contracts.md`) found the official client never sends either shape. `dbt.listNodes` takes `[selector]`, a dbt selector string built from the file's project-relative path (`+models/child.sql+` for parents+children at depth 0), not `[]`. `dbt.getCurrentNode` takes `[relativePath]`, a bare relative-path string with no line/character at all, not `{uri,line,character}`. `fusionEditorFlowsCapture.test.ts`'s `listNodesOfficialShape`/`getCurrentNodeOfficialShape` probes confirm both return a real, non-null object on this same loaded project once called with those shapes.
- `textDocument/hover` on `ref("base")` → real content (`"**Children Models**\n\n- child"`). Hover on a macro call (`example()`, no package qualifier) on the same loaded project → `null`. Hover works for `ref()`/`source()` targets but not for any macro invocation tried, dotted or not. Static inspection of the official bundle found no `middleware.provideHover`, no macro-specific request, and no custom `dbt/*` hover method — hover is plain, unmodified `textDocument/hover` — so this `null` is not explained by a client-side path the official client has and Fusion Power User lacks. Whether the official client's own UI shows the same `null` for this call was not observed live (no control run performed); "server limitation" therefore remains unresolved pending that control, not confirmed.
