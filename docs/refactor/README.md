...
The product runtime remains manifest-driven. Telemetry, AI, MCP, collaboration, credits, and notebooks are gone; hosted Altimate, authentication, and Python-bridge paths remain until Phase 8.2. Core and Cloud are no longer constructible. `DBTProjectIntegrationAdapter` is retired from production code; manifest parsing and explicit post-command `run_results.json` reads live in `FusionProjectIntegration`.
