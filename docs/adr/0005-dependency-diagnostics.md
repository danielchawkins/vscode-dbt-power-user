...
**Status:** D3 provisional. The 2026-09-22 S1 rerun established a parse-clean control (`dbt deps` and `dbt parse` exit 0 with valid base/child root models and an installed local package whose source lives outside the project root) but observed no positive loaded-project probe on control and no `textDocument/publishDiagnostics` on any arm. Neither confinement to the root project nor a root `dbt_project.yml` blocker is established.
...
**D3 remains provisional** (`status: "provisional"`, `reason: "control load probes did not reach success"`). This capture establishes a parse-clean control and records concrete null outcomes for capable load probes, but not a loaded LSP session; the dependency-failure and filtering branches were not evaluated. Step 5.5 should wait for a capture that reaches a positive loaded control or for consumer soak that observes dependency diagnostics in a loaded project.
