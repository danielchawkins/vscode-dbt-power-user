...
explicit path setting, and neither the extension nor its tests may invoke mise. The lock records checksums for both
supported macOS Fusion archives, and `just check` validates both npm lockfiles against HTTPS public-registry URLs and
integrity hashes.
